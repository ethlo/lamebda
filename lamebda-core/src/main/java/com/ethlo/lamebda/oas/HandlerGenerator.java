...
{
    // specification/oas.yaml -g spring --library spring-mvc -o /tmp/foo/
    // -DdelegatePattern=true,hideGenerationTimestamp=true,java8=true,performBeanValidation=true,useBeanValidation=true,dateLibrary=java8,
    // apiPackage=spec,modelPackage=spec,useDelegate=true,useTags=true,apiFirst=true

    private static final Logger logger = LoggerFactory.getLogger(HandlerGenerator.class);

    public URL generateApi(final Path specPath) throws IOException
    {
        ...
        configurator.addAdditionalProperty("api", false);
        configurator.addAdditionalProperty("java8", true);
        configurator.addAdditionalProperty("apifirst", true);
